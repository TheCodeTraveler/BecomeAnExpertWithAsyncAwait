...
// Workshop plumbing: runs the steps against your News page and keeps their results.
// Every step renders its own copy of the page against an in-memory Hacker News, so a check never touches the Top stories page you have open, or the real API.
// Only one step runs at a time, so a step that times Blazor's renderer never competes with another run for thread pool threads.
public sealed class StepVerifier(ILogger<StepVerifier> logger, IHostApplicationLifetime applicationLifetime) : BackgroundService
{
	...
	readonly StepState[] _states = [.. Enumerable.Repeat(new StepState(StepStatus.NotRun, null), 5)];
	...
	[
		new Step1LoadStoriesDuringInitialization(),
		new Step2KeepTheRendererFree(),
		new Step3StopWhenThePageGoesAway(),
		new Step4StreamStories(),
		new Step5SkipUnneededStateMachines(),
	];
	...
	async Task CheckSteps(CancellationToken token)
	{
		try
		{
			// Results from before the latest change to your code are not trusted
			lock (_lock)
			{
				Array.Fill(_states, new StepState(StepStatus.NotRun, null));
			}

			logger.LogInformation("Checking your News page against every step. Open the app in your browser: the workshop guide beside the Top stories page shows what to fix next");
