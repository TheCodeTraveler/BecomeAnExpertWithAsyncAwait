...
// Every step creates its own instances of the services, so a check never disturbs the singletons the Checkout page uses.
...
public sealed class StepVerifier(ILogger<StepVerifier> logger, IHostApplicationLifetime applicationLifetime) : BackgroundService
{
	...
	[
		new Step1CountEveryOrder(),
		new Step2AddUpRevenue(),
		new Step3BuildTaxTableOnce(),
		new Step4ReserveStock(),
	];
	...
	async Task CheckSteps(CancellationToken token)
	{
		try
		{
			...
			logger.LogInformation("Checking your services against every step. Open the app in your browser: the workshop guide beside the Checkout page shows what to fix next");
