...
// Workshop plumbing: runs the steps against your Product page and keeps their results.
// Every step renders its own copy of the page with its own instances of the services, so a check never disturbs the singletons the Product page uses.
// Only one step runs at a time, so a step that measures how long the page takes never competes with another run for thread pool threads.
public sealed class StepVerifier(ILogger<StepVerifier> logger, IHostApplicationLifetime applicationLifetime) : BackgroundService
{
	...
	[
		new Step1StartEveryCall(),
		new Step2ContainEachFailure(),
		new Step3PaintEachCard(),
	];
	...
	async Task CheckSteps(CancellationToken token)
	{
		try
		{
			// Results from before the latest change to the page are not trusted
			lock (_lock)
			{
				Array.Fill(_states, new StepState(StepStatus.NotRun, null));
			}

			logger.LogInformation("Checking your Product page against every step. Open the app in your browser: the workshop guide beside the Product page shows what to fix next");
