...
// Workshop plumbing: runs the steps against your Dashboard code and keeps their results.
// Every step renders its own Dashboard with its own MarketDataService, so a check never disturbs the page open in your browser.
// Only one step runs at a time, so a step that measures contention never competes with another run for thread pool threads.
public sealed class StepVerifier(ILogger<StepVerifier> logger, IHostApplicationLifetime applicationLifetime) : BackgroundService
{
	...
	[
		new Step1ShowEveryCard(),
		new Step2KeepEveryQuote(),
		new Step3CountEveryQuote(),
		new Step4GuardRefreshTimer(),
	];
	...
	async Task CheckSteps(CancellationToken token)
	{
		try
		{
			// Results from before the latest change to the code are not trusted
			lock (_lock)
			{
				Array.Fill(_states, new StepState(StepStatus.NotRun, null));
			}

			logger.LogInformation("Checking your Dashboard code against every step. Open the app in your browser: the workshop guide beside the Dashboard page shows what to fix next");
