...
// Every step creates its own instances of the services, so a check never disturbs the singletons the Ingest page uses.
// Only one step runs at a time, so a step that measures how long something takes never competes with another run.
public sealed class StepVerifier(ILogger<StepVerifier> logger, IHostApplicationLifetime applicationLifetime) : BackgroundService
{
	...
	readonly StepState[] _states = [.. Enumerable.Repeat(new StepState(StepStatus.NotRun, null), 4)];
	...
	[
		new Step1AnswerRightAway(),
		new Step2QueueIntoBoundedChannel(),
		new Step3DrainWithSeveralConsumers(),
		new Step4DrainBacklogOnShutdown(),
	];
	...
	// Finds the member named by a stub's NotImplementedException.
	...
	static string? FindMissingMember(Exception exception)
	{
		...
		for (var current = exception; current is not null; current = current.InnerException)
		{
			...
			if (current.TargetSite is { DeclaringType: { } declaringType } method && declaringType.Assembly == typeof(StepVerifier).Assembly)
				return method.IsSpecialName ? $"{declaringType.Name}.{method.Name.Replace("get_", string.Empty, StringComparison.Ordinal)}" : $"{declaringType.Name}.{method.Name}()";
		}
	...
	async Task CheckSteps(CancellationToken token)
	{
		try
		{
			...
			logger.LogInformation("Checking your services against every step. Open the app in your browser: the workshop guide beside the Ingest page shows what to fix next");
	...
	async Task<StepState> Execute(WorkshopStep step, StepReport report, Task stopRequested, CancellationToken runToken, CancellationToken token)
	{
		try
		{
			// A dedicated thread runs the step until its first await. If your code blocks it forever,
